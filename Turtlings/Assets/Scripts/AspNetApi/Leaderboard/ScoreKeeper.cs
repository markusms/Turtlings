...
public class ScoreKeeper
{
    ...
    { //read the saved scores from a file
        string line;
        List<Score> scores = new List<Score>(); //create a new list that saves Score class objects
        using (StreamReader readtext = new StreamReader("scores.txt"))
        { //Read file "scores.txt"
            while ((line = readtext.ReadLine()) != null)
            { //read as long as the file has lines
        ...
        { //iterate through the scores list
            if (positionOnTheLeaderboard > 10)
            { //max 10 results
                break;
            }
            else if (positionOnTheLeaderboard == 10)
            { //if the 10th result then don't add the empty spaces "  " to the start of the string to make the leaderboard look better
        ...
        while (positionOnTheLeaderboard < 10)
        {//make the list always 10 results long to make it look better
            if (positionOnTheLeaderboard == 9)
            { //if the 10th result then don't add the empty spaces "  " to the start of the string to make the leaderboard look better
